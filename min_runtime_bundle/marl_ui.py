...
from marl_common import CFG, IState, EState, IRole, EType, dist2d
...
class DemoRenderer:
    def __init__(self, env, fullscreen=False, ui_style="arc"):
        ...
        self.ui_style = ui_style if ui_style in ("arc", "rect") else "arc"
    ...
    def _ecol(self,e):
        if e['state']==EState.DESTROYED: return self.C['ed']
    ...
    def render(self,spd=4):
        pg=self.pg; self.fc+=1; self.pulse=(self.pulse+0.025)%(2*math.pi)
        self._fps_c+=1; now=time.time()
        if now-self._fps_t>=0.5: self._fps=int(self._fps_c/(now-self._fps_t)); self._fps_c=0; self._fps_t=now
    ...
    def _draw_enemy_hud(self):
        ...
        active = [e for e in self.env.enemies if e['state'] in (EState.APPROACHING, EState.MANEUVERING)]
        total_enemies = len([e for e in self.env.enemies if e['state'] != EState.DESTROYED])
        ...
        for idx, enemy in enumerate(active):
            ...
            sector = self._sector_label(enemy['x'])
            dist = max(0.0, CFG.INTERCEPT_FAIL_LINE - enemy['y'])
    ...
    def _draw_net_overlay(self):
        ...
        for enemy_id, team in barrier_map.items():
            ...
            points = [self._w2s(it['x'], it['y']) for _, it in members]
    ...
    def _draw_entities(self):
        ...
        for e in self.env.enemies:
            ...
            if self.show_poi:
                for role,iid in[('p',pid),('f',fid)]:
                    ...
                    if it.get('poi'):
                        ix,iy=self._w2s(it['x'],it['y']); px,py=self._w2s(it['poi'][0],it['poi'][1])
            ...
            if pid is not None and fid is not None:
                ...
                if pi and fi and pi['state'] not in (IState.DESTROYED,IState.STANDBY) and fi['state'] not in (IState.DESTROYED,IState.STANDBY):
                    p1,p2=self._w2s(pi['x'],pi['y']),self._w2s(fi['x'],fi['y'])
        ...
        for e in self.env.enemies:
            ...
            if len(trail) > 1 and e['state'] != EState.DESTROYED:
            ...
            if e['y'] > 8000 or e['state'] == EState.DESTROYED: brightness = 1.0
            base_c = self._ecol(e)
            ...
            if e['state'] in (EState.APPROACHING, EState.MANEUVERING):
                ...
                if brightness > 0.3:
                    ...
                    lbl = f"F-{e['id'] + 1} {self._sector_label(e['x'])}"
                    self.screen.blit(self.fms.render(lbl, True, c), (sx + 10, sy - 6))

            elif e['state'] == EState.DESTROYED:
                pg.draw.line(self.screen, c, (sx - 6, sy - 6), (sx + 6, sy + 6), 2)
                pg.draw.line(self.screen, c, (sx + 6, sy - 6), (sx - 6, sy + 6), 2)

        # 绘制拦截机
        for it in self.env.interceptors:
            if it['state'] in (IState.STANDBY, IState.LANDED): continue
            if it['state']==IState.DESTROYED: continue

            sx,sy=self._w2s(it['x'],it['y']); c=self._icol(it)
            r=self._screen_heading(it['x'], it['y'], it['heading'])
            sz=14
            ...
            # === 锁敌连线 ===
            if it['state'] in (IState.INTERCEPTING, IState.FOLLOWING) and it['target_id'] is not None:
    ...
    def _draw_hdr(self,spd):
        ...
        ui_mode = "ARC" if self.ui_style == "arc" else "RECT"
        ...
        bx=self.p2x; rem=max(0,CFG.TIME_LIMIT-self.env.time)
        m,s=int(rem//60),int(rem%60); tc=self.C['red'] if rem<60 else self.C['txt']
        ...
        self.screen.blit(self.fcn.render(f"T-{m:02d}:{s:02d}",True,tc),(bx+10,20))
    ...
    def _draw_p2(self):
        ...
        standby_count = sum(1 for it in self.env.interceptors if it['state'] in (IState.STANDBY, IState.LANDED))
        airborne_count = sum(
            1
            for it in self.env.interceptors
        ...
        count_surf = self.fms.render(
            f"{len(self.env.interceptors)} | 待{standby_count} 空{airborne_count} 保{reserved_count}",
            True,
            self.C['green'],
        )
        self.screen.blit(count_surf, (px + pw - count_surf.get_width() - 8, py + 11))
        pg.draw.line(self.screen, self.C['cyd'], (px + 8, py + header_h), (px + pw - 8, py + header_h), 1)
        cols = 2; card_w = (pw - 20) // 2; card_h = 92
        gap_y = 4
        view_y = py + header_h + 4
        ...
        for idx, it in enumerate(self.env.interceptors):
            ...
            if it.get('jammed_by_interference'):
                ...
                st, sc = stm.get(it['state'], ("--", self.C['txtd']))
            ...
            self.screen.blit(self.fms.render(mission_text, True, c), (cx+5, cy+60))
        ...
        self.screen.set_clip(prev_clip)
        total_rows = (len(self.env.interceptors) + cols - 1) // cols
