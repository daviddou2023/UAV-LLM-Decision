"""
Whisper 常驻推理服务 - ui/whisper_server.py
用途: 模型只加载一次，常驻 GPU 显存，主程序随便重启都秒连可用。

启动:
    python ui/whisper_server.py                    # 默认 small 模型
    python ui/whisper_server.py --model medium     # 用 medium 模型
    python ui/whisper_server.py --port 5555        # 指定端口
